...
ITEMS_FILTERED_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'items_filtered.json')
...
def get_all_manufacturable_item_ids():
    """Henter alle typeIDs for varer som kan produseres, basert på SDE."""
    conn = connect_to_sde()
    if not conn: return []
    try:
        cursor = conn.cursor()
        # Henter produkt-typeID for alle publiserte tegninger
        cursor.execute("""
            SELECT T.typeID
            FROM invTypes T
            JOIN industryActivityProducts P ON T.typeID = P.productTypeID
            JOIN invTypes BPO ON P.typeID = BPO.typeID
            WHERE BPO.published = 1 AND T.published = 1 AND P.activityID = 1
        """)
        return [row[0] for row in cursor.fetchall()]
    except sqlite3.Error as e:
        logging.error(f"SQL-feil ved henting av produserbare vare-IDer: {e}")
        return []
    finally:
        if conn:
            conn.close()

def get_filtered_item_ids(status_callback=None):
    """Laster en liste med vare-IDer fra items_filtered.json ved å konvertere navn til ID-er."""
    
    if status_callback:
        status_callback("Bygger ID-map fra SDE...", 1)
        
    name_to_id_map = get_all_item_name_id_map()
    if not name_to_id_map:
        logging.error("Kunne ikke bygge item name-to-ID map.")
        return []

    try:
        with open(ITEMS_FILTERED_PATH, 'r') as f:
            item_names = json.load(f)
        
        if status_callback:
            status_callback(f"Laster {len(item_names)} varenavn...", 2)

        item_ids = []
        total_names = len(item_names)
        for i, name in enumerate(item_names):
            item_id = name_to_id_map.get(name.lower())
            if item_id:
                item_ids.append(item_id)
            
            if status_callback and i > 0 and i % 500 == 0:
                progress = 2 + int((i / total_names) * 3) # Skalerer fremgang fra 2% til 5%
                status_callback(f"Konverterer... ({i}/{total_names})", progress)

        return item_ids
    except FileNotFoundError:
        logging.error(f"Feil: 'items_filtered.json' ble ikke funnet på stien: {ITEMS_FILTERED_PATH}")
        return []
    except json.JSONDecodeError:
        logging.error("Feil: Kunne ikke dekode 'items_filtered.json'. Filen kan være korrupt.")
        return []
...
def get_all_item_name_id_map():
    """Henter en map med alle publiserte varenavn (lowercase) til typeID fra SDE."""
    conn = connect_to_sde()
    if not conn: return {}
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT typeName, typeID FROM invTypes WHERE published = 1")
        # Bruker .lower() for å sikre case-insensitiv matching senere
        return {row[0].lower(): row[1] for row in cursor.fetchall()}
    except sqlite3.Error as e:
        logging.error(f"SQL-feil ved henting av item name/id map: {e}")
        return {}
    finally:
        if conn:
            conn.close()
...
def get_item_id_by_name(item_name):
    """Henter varens typeID basert på navnet, case-insensitivt."""
    conn = connect_to_sde()
    if conn is None:
        return None
    try:
