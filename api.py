...
def get_market_prices(type_ids, station_id=None, region_id=None):
    ...
    prices = defaultdict(lambda: {'buy': 0.0, 'sell': 0.0})
...
def get_scanner_market_data(type_ids, station_id=None, region_id=None):
    ...
    market_data_map = defaultdict(lambda: {
        'highest_buy': 0.0, 'lowest_sell': 0.0, 'buy_order_volume': 0, 'sell_order_volume': 0
    })
    id_chunks = [unique_type_ids[i:i + 200] for i in range(0, len(unique_type_ids), 200)]
    for chunk in id_chunks:
        try:
...
def get_location_names(location_ids, access_token, character_id):
    ...
    location_ids = [loc_id for loc_id in location_ids if loc_id is not None]
    if not location_ids:
        logging.debug("get_location_names called with empty location_ids list.")
        return {}
    if isinstance(location_ids, set):
        location_ids = list(location_ids)
    logging.debug(f"get_location_names called with IDs: {location_ids}")
    logging.debug(f"Access token present: {bool(access_token)}")
    names = {}
    # Separate structure and station IDs
    structure_ids = [loc_id for loc_id in location_ids if isinstance(loc_id, int) and loc_id >= 100000000000]
    station_ids = [loc_id for loc_id in location_ids if isinstance(loc_id, int) and loc_id < 100000000000]
    # Resolve station names via /universe/names/
    id_chunks = [station_ids[i:i + 1000] for i in range(0, len(station_ids), 1000)]
    for chunk in id_chunks:
    ...
    # Resolve structure names one by one (ESI does not support bulk)
    if structure_ids and access_token:
        for struct_id in structure_ids:
            try:
                details = get_structure_details(struct_id, access_token)
                if details and 'name' in details:
                    names[struct_id] = details['name']
                else:
                    logging.warning(f"Structure ID {struct_id} could not be resolved.")
            except Exception as e:
                logging.error(f"Error resolving structure {struct_id}: {e}")
    # Fallback for unresolved IDs
    for loc_id in location_ids:
        if loc_id not in names:
            if loc_id == character_id:
                names[loc_id] = "Asset Safety"
            elif isinstance(loc_id, int):
                id_type = 'structure' if loc_id >= 100000000000 else 'station'
                logging.warning(f"Location ID {loc_id} not found in names mapping. Likely a {id_type}.")
                names[loc_id] = f"Unknown Location ({loc_id})"
            else:
                logging.warning(f"Location ID {loc_id} is not a valid int and cannot be resolved.")
                names[loc_id] = f"Unknown Location ({loc_id})"
    logging.debug(f"get_location_names result: {names}")
    return names
...
def open_market_window(type_id, access_token, status_callback=None):
    """Åpner markedsdetaljer for en gitt vare-ID i spillet."""
    ...
    if response and response.status_code == 204:
        logging.info(f"Markedsvindu-kommando sendt for type_id: {type_id}")
        return True
    else:
        status = response.status_code if response else "N/A"
        logging.error(f"Kunne ikke åpne markedsvindu for {type_id}. Status: {status}")
        return False

def add_waypoint(destination_id, access_token, clear_other_waypoints=False, add_to_beginning=False):
    """Legger til et veipunkt i autopiloten i spillet."""
    ...
    if response and response.status_code == 204:
        logging.info(f"Veipunkt satt for destinasjon: {destination_id}")
        return True, f"Veipunkt til {destination_id} ble satt."
    else:
        status = response.status_code if response else "N/A"
        error_message = f"Kunne ikke sette veipunkt for {destination_id}. Status: {status}"
