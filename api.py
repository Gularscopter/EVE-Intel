...
_structure_details_cache = {} # Ny cache for strukturer
CACHE_TIMEOUT_SECONDS = 3600 # 1 hour

def verify_token_and_get_character(access_token):
    """Verifies a token and returns character ID and Name."""
...
def get_structure_name(structure_id, access_token):
    """Henter navnet på en spiller-eid struktur fra ESI."""
    now = time.time()
...
def resolve_name_to_id(name, category=None):
    ...
    try:
        response = requests.post(f"{ESI_BASE_URL}/universe/ids/", json=[name])
        response.raise_for_status()
        data = response.json()
        if category:
            key = category + 's'
            if key in data and data[key]:
                result_id = data[key][0]['id']
                _name_cache[cache_key] = result_id
                return result_id
...
def get_station_details(station_id):
    ...
    try:
        station_response = requests.get(f"{ESI_BASE_URL}/universe/stations/{station_id}/")
        station_response.raise_for_status()
        station_data = station_response.json()
        system_response = requests.get(f"{ESI_BASE_URL}/universe/systems/{station_data['system_id']}/")
        system_response.raise_for_status()
        constellation_id = system_response.json()['constellation_id']
        constellation_response = requests.get(f"{ESI_BASE_URL}/universe/constellations/{constellation_id}/")
...
def get_corp_name(corp_id):
    response = requests.get(f"{ESI_BASE_URL}/corporations/{corp_id}/")
    return response.json().get('name', 'N/A') if response.status_code == 200 else 'N/A'

def get_alliance_name(alliance_id):
    response = requests.get(f"{ESI_BASE_URL}/alliances/{alliance_id}/")
    return response.json().get('name', 'N/A') if response.status_code == 200 else 'N/A'
...
def get_item_price_history(region_id, type_id):
    try:
        response = requests.get(f"{ESI_BASE_URL}/markets/{region_id}/history/?type_id={type_id}")
        response.raise_for_status()
        return response.json()
