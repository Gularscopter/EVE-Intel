...
class AuthManager:
    ...
    def get_current_character_location(self):
        token = self.get_valid_token()
        char_id = self.character_info.get('id')
        if not token or not char_id:
            return None
        
        system_id = api.get_character_current_system_id(char_id, token)
        if not system_id:
            return None
    ...
    def get_current_character_system_id(self):
        token = self.get_valid_token()
        char_id = self.character_info.get('id')
        if not token or not char_id:
            return None
        return api.get_character_current_system_id(char_id, token)
